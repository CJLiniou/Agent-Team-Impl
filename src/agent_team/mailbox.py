...
class AgentMailbox(SqliteStore["Message"]):
    """线程安全的邮箱，用于智能体间通信。
    ...
    def __init__(self, db_path: Path):
        ...
        self._init_db()
        self._notify_callback = None  # async (recipient_name, sender_name) -> None

    def set_notify_callback(self, callback):
        """设置新消息通知回调。回调会在 send/broadcast 后调用。"""
    ...
    def send(self, sender: str, recipient: str, subject: str, content: str,
        ...
        if self._notify_callback:
            ...
            try:
                ...
                if loop.is_running():
                    asyncio.create_task(self._notify_callback(
                        recipient or "__broadcast__", sender
                    ))
            except RuntimeError:
                pass
        return msg
    ...
    def list_messages(self, limit: int = 200) -> list[Message]:
        ...
        with self._get_conn() as conn:
            rows = conn.execute(
                "SELECT * FROM mailbox ORDER BY created_time DESC LIMIT ?",
                (limit,)
            ).fetchall()
            return [self._row_to_object(row) for row in rows]
        ...
        with self._get_conn() as conn:
            conn.execute(
                ...
                (msg.id, msg.sender, msg.recipient, msg.subject, msg.content,
                 ...
                 self._serialize_meta(msg.metadata), now)
            )

    def _row_to_object(self, row: sqlite3.Row) -> Message:
        """将数据库行转换为 Message 对象。"""
        return Message(
            id=row['id'],
            sender=row['sender'],
