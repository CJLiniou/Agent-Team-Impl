...
class TaskManager(SqliteStore["Task"]):
    ...
    def __init__(self, db_path: Path, hook_registry=None):
        ...
        self._notify_callback = None  # async (agent_name, task_name) -> None
    ...
    def create_task(self, name: str, description: str = "", assigned_to: str = "",
        ...
        if task.depends_on:
            ...
            if not self._are_dependencies_satisfied(task.id):
    ...
    def update_task_status(self, task_id: str, status: TaskStatus,
        ...
        # 如果任务完成，自动解除依赖此任务的所有阻塞任务
        if ok and status == TaskStatus.COMPLETED:
            self._unblock_dependents(task_id)

        return ok

    def _unblock_dependents(self, completed_task_id: str) -> None:
        """当任务完成时，检查并解除所有依赖它的阻塞任务。

        对每个依赖 completed_task_id 的 BLOCKED 任务，
        检查其所有依赖是否均已满足，若满足则转为 PENDING。
        """
        with self._get_conn() as conn:
            # 找到所有依赖此任务的任务
            rows = conn.execute(
                """SELECT td.task_id FROM task_dependencies td
                   ...
                   WHERE td.depends_on_id = ? AND t.status = ?""",
                (completed_task_id, TaskStatus.BLOCKED.value),
            ).fetchall()
            ...
            for row in rows:
                ...
                if self._are_dependencies_satisfied(task_id):
                    ...
                    # 通知被解除阻塞的 Agent
                    task = self.get_task(task_id)
                    if task and task.assigned_to and self._notify_callback:
                        import asyncio
                        asyncio.create_task(
                            self._notify_callback(task.assigned_to, task.name)
                        )
                    logger.info(
                        f"Task {task_id[:8]}... unblocked (dependency {completed_task_id[:8]}... completed)"
                    )
            conn.commit()
    ...
    def _are_dependencies_satisfied(self, task_id: str) -> bool:
        ...
        with self._get_conn() as conn:
            ...
            for row in rows:
                ...
                # Terminal states: COMPLETED (normal) or FAILED (don't deadlock)
                if not dep_status:
                    return False
                if dep_status['status'] not in (TaskStatus.COMPLETED.value, TaskStatus.FAILED.value):
                    return False
